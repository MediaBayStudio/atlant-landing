...
(function() {
  ...
  if ($quizBlock) {
      ...
      */
      Quiz.prototype.nextStep = function() {
        ...
        if (options.next) {
          ...
          for (let i = 0, len = _.$fieldsOnThisStep.length; i < len; i++) {
            if (_.$fieldsOnThisStep[i].classList.contains('required')) {
              breaked = true;
              break;
            }
          }
      ...
      Quiz.prototype.prevStep = function() {
        ...
        if (options.next) {
          ...
          for (let i = 0, len = _.$fieldsOnThisStep.length; i < len; i++) {
            // console.log(_.$fieldsOnThisStep[i]);
            if (_.$fieldsOnThisStep[i].classList.contains('required')) {
              breaked = true;
              break;
            }
          ...
          // console.log(_.$fieldsOnThisStep);
          // console.log(breaked);
      ...
      Quiz.prototype.resetQuiz = function() {
        ...
        if (_.options.counter) {
          _.$counterCurrentNum.textContent = _.currentStep + 1;
        }

        if (_.options.dots) {
          for (let i = 0, len = _.$dots.length; i < len; i++) {
        ...
        // if (_.options.steps[_.currentStep].options.img) {
        //   let image = _.options.steps[_.currentStep].options.img,
    ...
    let xhr = new XMLHttpRequest(),
      imagePath = templateDirectoryUri + '/img/',
    ...
    xhr.open('post', templateDirectoryUri + '/quiz-kredit-pod-zalog.json');
    ...
    $quizBlock.addEventListener('nextstep', function(event) {
      ...
      scrollToTarget(event, $quizBlock);
    });
    ...
    xhr.addEventListener('readystatechange', function() {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        let quiz = new Quiz($quizBlock, {
          ...
          dots: true,
          counter: true,
          nextBtnClass: 'quiz__next btn btn_blue',
          nextBtnText: 'Дальше',
          $form: $quizForm,
          $result: $quizResult
        });
